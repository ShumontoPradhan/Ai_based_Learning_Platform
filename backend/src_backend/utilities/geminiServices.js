 ...
 * @param {string} text- Document text
 * @param {number} count- Number of flashcards to generate
 * @returns {Promise<Array<question: string,options: Array, correctAnswer string, explain: String, difficulty: string}>>}
 */
...
`;
        ...
        console.error('Gemini API error:', error);
        return []; // never throw → controller handles safely
    }
};
...
/**
 * @desc Chat with document context
 * @param {string} text- User questions
 ...
 * @param {string} text- Relevant context
 * @returns {Promise<string>}
 */
